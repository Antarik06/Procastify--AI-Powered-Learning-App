...
const QuizPage: React.FC<QuizProps> = ({ notes, user, stats, setStats }) => {
    ...
    const handleGenerate = async () => {
        ...
        if (aggregatedText.length < 50) {
            notify.info("The selected notes don't contain enough text content to generate a quiz.");
            setLoading(false);
            return;
        }
        ...
        if (questions.length === 0) {
            notify.error("Failed to generate questions. Please try a different note or try again.");
            setLoading(false);
            return;
        }
    ...
    const handleJoinQuiz = async () => {
        if (!joinCode.trim()) {
            notify.info('Please enter a quiz code');
            return;
        }
        ...
        try {
            ...
            if (!session) {
                notify.error('Invalid quiz code or quiz already started');
            ...
            if (session.participants.some(p => p.userId === user.id)) {
                notify.success('You have already joined this quiz');
                setMultiplayerSession(session);
                setView('waiting');
                setJoiningSession(false);
                return;
            ...
            notify.error(error.message || 'Failed to join quiz');
            setJoiningSession(false);
        }
    };
    ...
    const handleContinueQuiz = async () => {
        ...
        if (newQuestions.length > 0 && quiz) {
            ...
            notify.info("Could not generate more questions for this content.");
        }

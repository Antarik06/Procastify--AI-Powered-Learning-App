...
const StudentClassroomView: React.FC<StudentClassroomViewProps> = ({ user, classroomId, onNavigate }) => {
  ...
  const handleViewResource = async (resource: ClassroomResource) => {
    try {
      ...
      if (noteSnap.exists()) {
        ...
        notify.error('Note not found. It may have been deleted by the teacher.');
      ...
      notify.error(`Failed to load note: ${error.message || 'Unknown error'}`);
      setSelectedResource(null);
    } finally {
  ...
  const handleCopyToMyNotes = async () => {
    ...
    try {
      ...
      notify.success('Note copied to your notes!');
      setSelectedResource(null);
      setResourceNote(null);
    } catch (error) {
      console.error('Error copying note:', error);
      notify.error('Failed to copy note');
    } finally {
      setCopying(false);
    }
  };

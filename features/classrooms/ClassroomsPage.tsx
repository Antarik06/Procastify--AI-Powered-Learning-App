...
const Classrooms: React.FC<ClassroomsProps> = ({ user, onNavigate }) => {
  ...
  const handleCreateClassroom = async () => {
    ...
    if (user.isGuest) {
      notify.error('Guest users cannot create classrooms. Please sign up first.');
      return;
    }
    ...
    try {
      ...
      notify.error(`Failed to create classroom: ${error.message || 'Unknown error'}`);
    } finally {
      setCreating(false);
    }
  };
